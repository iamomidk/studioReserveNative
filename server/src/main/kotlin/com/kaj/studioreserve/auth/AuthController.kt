...
class AuthController(
    ...
    fun refresh(refreshToken: String): AuthResponse {
        ...
        val tokenType = decoded.getClaim(JwtService.Companion.CLAIM_TOKEN_TYPE).asString()
        if (tokenType != TokenType.REFRESH.claimValue) {
            throw AuthException(HttpStatusCode.BadRequest, "Invalid refresh token type")
        }

        val tokenId = decoded.getClaim(JwtService.Companion.CLAIM_TOKEN_ID).asString()
